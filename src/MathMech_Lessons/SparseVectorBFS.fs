...
type SparseVectorBFS<'value when 'value: equality> =
    ...
    member this.Item
        with get i =
            let vectorElement i (vector: SparseVectorBFS<'value>) =
...
let addVector (fPlus: option<'value1> -> option<'value2> -> option<'value3>) (vector1: SparseVectorBFS<'value1>) (vector2: SparseVectorBFS<'value2>) : SparseVectorBFS<'value3> =
    ...
    if vector1.Length = vector2.Length then
        SparseVectorBFS(addTrees vector1.Storage vector2.Storage, vector1.Length)
    else
