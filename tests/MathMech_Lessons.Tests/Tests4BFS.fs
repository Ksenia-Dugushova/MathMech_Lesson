    ...
    let tests =
        testList
            ...
            [ testCase "function square for one element 1"
              ...
              <| fun _ ->
                  let actualResult =
                      Expect.throws (fun _ -> SparseVectorBFS([||])[152u] |> ignore) "Index out of the range"

                  actualResult

              testProperty "vectorElement property test"
              <| fun (arr: array<int option>) (i: uint) ->
                  let actualResult = SparseVectorBFS arr
    ...
    let tests =
        testList
            ...
            [ testCase "Multi vector and matrix"
              <| fun _ ->
                  let vec = SparseVectorBFS([| Some(0); Some(1) |])
              ...
              <| fun _ ->
                  let vec = SparseVectorBFS([| Some(1); Some(1); Some(1) |])
              ...
              <| fun (x: uint) (y: uint) ->
                  if x <> 0u && y <> 0u then
                      ...
                      let vec = SparseVectorBFS arrSome
                      ...
                      let expectedResult =
                          naiveMulti arr arr2d |> Array.map zeroToSomeNone |> SparseVectorBFS
...
module PropertyTests =
    ...
    let vectorsMaker (length: int) =
        ...
        SparseVectorBFS arrOfSome, arrOfSome
        ...
        testList
            ...
            [ testProperty "Sum vectors"
              <| fun (length: int) ->
                  ...
                  <| SparseVectorBFS(naiveSum arrOfSome1 arrOfSome2).Storage
